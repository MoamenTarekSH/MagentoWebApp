...
public class T06_VerifyPruchesItemUsingRegistredEmailId extends BaseTest 
{
	HomePage homeObj;
	...
	/*
	 * @Test(priority = 0) public void Test_Register_Account() throws
	 * InterruptedException { homeObj = new HomePage(driver);
	 * homeObj.Open_Register_Page(); registerObj = new RegisterPage(driver);
	 * registerObj.Enter_Register_Data(F_Name, M_Name, L_Name, E_mail, Pass , Pass);
	 * Assert.assertTrue(registerObj.AssertSuccessReg_MSG.getText().
	 * contains("Thank you for registering with Main Website Store."));
	 * System.out.println("Email===>"+E_mail); System.out.println("Pass===>"+Pass);
	 * }
	 * 
	 * @Test(priority = 1) public void Test_AddItemToWishList() { homeObj = new
	 * HomePage(driver); homeObj.Opene_TV_Page(); tvObj = new TVPage(driver);
	 * tvObj.OpenDetail_Item1(); aboutObj = new AboutItemPage(driver);
	 * aboutObj.AddItemToWishList(); wishObj = new WishListPage(driver);
	 * Assert.assertTrue(wishObj.AssertAddWish_Msg.getText().
	 * contains("has been added to your wishlist")); }
	 */
